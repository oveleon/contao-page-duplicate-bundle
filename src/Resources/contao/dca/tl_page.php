...
$GLOBALS['TL_DCA']['tl_page']['config']['onload_callback'][] = ['tl_page_duplicate', 'makePageDuplicateMandatory'];
 ...
 */
class tl_page_duplicate extends Backend
{
    /**
     ...
     * @throws Exception
     */
    public function makePageDuplicateMandatory(DataContainer $dc)
    {
        $objPage = $this->Database->prepare("SELECT * FROM " . $dc->table . " WHERE id=?")
            ->limit(1)
            ->execute($dc->id);

        if ($objPage->numRows && 'duplicate' == $objPage->type)
        {
